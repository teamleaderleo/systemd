...
GUEST_SOURCE=/usr/share/minimal_0.raw
if [[ ! -f "$GUEST_SOURCE" ]]; then
    echo "minimal_0.raw not found, skipping"
    exit 0
fi
...
at_exit() {
    ...
    mountpoint -q "$GUEST_ROOT" && umount "$GUEST_ROOT"
    rm -rf "$WORKDIR"
...
# minimal_0.raw is the squashfs root artifact intentionally installed into the
# general systemd test image. Mount it in the outer test VM, then copy that full
# root into the same plain-ext4 disk format already used by TEST-87 vmspawn tests.
mount -t squashfs -o loop,ro "$GUEST_SOURCE" "$GUEST_ROOT"
test -x "$GUEST_ROOT/usr/bin/bash"
truncate -s 1G "$IMAGE"
mke2fs -t ext4 -q -d "$GUEST_ROOT" "$IMAGE"
umount "$GUEST_ROOT"
